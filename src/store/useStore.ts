...
- 用编辑和作者聊天的语气，不是填模板`,
  ...
  framework: `你是文章结构设计师。上一步材料分析提出了一个核心问题和若干加固建议。

关键规则：如果用户在编辑器中声明了文章意图（例如"写一篇产品介绍文，面向普通读者"、"写一篇驳论文"、"写一篇教程"），请以用户意图为最高优先级——结构服务于用户的沟通目标，而非材料分析提出的核心问题。只有用户未声明意图时，才围绕材料分析的「## 核心问题」设计结构。
